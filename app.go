...
//go:embed content/blog/*.md public
var files embed.FS
...
func newApp() (*zinc.App, error) {
	content, err := fs.Sub(files, "content")
	if err != nil {
		return nil, err
	}
	blogService, err := blog.New(content)
	if err != nil {
		return nil, err
	}
	...
	images, err := fs.Sub(public, "image")
	if err != nil {
		return nil, err
	}

	app := zinc.New()
	if err := app.StaticFS("/assets", public); err != nil {
		return nil, err
	}
